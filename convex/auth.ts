...
function getAllowlistedEmails() {
  return buildAllowlist(String(process.env.BETA_ALLOWLIST_EMAILS ?? ""), getFounderEmail());
}
  ...
  callbacks: {
    async createOrUpdateUser(ctx, args) {
      ...
      const activeTesters = await db
        ...
        .collect();
      const eligibility = checkBetaSignupEligibility({
        email,
        allowlist: allowlistedEmails,
        activeTesterCount: countActiveTesters(activeTesters),
        maxTesters: getBetaMaxTesters(),
        isFounder,
      });
      if (!eligibility.allowed) {
        if (eligibility.reason === "not_allowlisted") {
