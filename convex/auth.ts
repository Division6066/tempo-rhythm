...
export const { auth, signIn, signOut, store, isAuthenticated } = convexAuth({
  ...
  callbacks: {
    async createOrUpdateUser(ctx, args) {
      ...
      const signupDecision = evaluateBetaSignup({
        email,
        allowlistedEmails,
        isFounder,
        activeTesterCount: countActiveBetaTesters(
          await db
            .query("users")
            .withIndex("by_betaAccess", (q) => q.eq("betaAccess", "tester"))
            .collect(),
        ),
        maxTesters: getBetaMaxTesters(process.env.BETA_MAX_TESTERS),
      });
      if (!signupDecision.allowed) {
        if (signupDecision.reason === "not_allowlisted") {
