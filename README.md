...
- **AI routing:** Mistral API via native `fetch` in `convex/lib/ai_router.ts` (`fast` / `balanced` / `deep` tiers). No provider SDKs. See `docs/HARD_RULES.md` §6.
...
- [`docs/CI.md`](./docs/CI.md) — GitHub Actions jobs, blocking vs notice-only checks, local equivalents.
