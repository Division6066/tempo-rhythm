...
- **AI routing:** Mistral API via native `fetch` in Convex actions (`convex/lib/ai_router.ts`); tiered models (`mistral-small-latest` → `mistral-large-latest`). See `docs/HARD_RULES.md` §2 and `docs/BRAIN_DUMP_TODAY.md`.
...
# 2. Install dependencies (root lockfile only — see docs/LOCAL_DEV_WINDOWS.md)
bun install
...
- [`docs/BRAIN_DUMP_TODAY.md`](./docs/BRAIN_DUMP_TODAY.md) — Today view + brain-dump planning loop (developer architecture).
- [`docs/SHIP_STATE.md`](./docs/SHIP_STATE.md) — what is shipped vs coded vs planned.
- [`docs/ENVIRONMENTS.md`](./docs/ENVIRONMENTS.md) — dev / test / preview / deployment contract.
