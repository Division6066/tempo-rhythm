...
- **AI routing:** Mistral API via native `fetch` in `convex/lib/ai_router.ts` (tiers: `fast` / `balanced` / `deep`). PRD may still reference OpenRouter as a future option — **implemented code wins** (see `docs/HARD_RULES.md` §2).
...
- **Package manager:** Bun 1.3.9 (`packageManager` in root `package.json`, lockfile: `bun.lock`)
