...
- **AI routing (implemented):** Mistral API via native `fetch` in `convex/lib/ai_router.ts` — tiered models (`mistral-small` / `mistral-medium` / `mistral-large`). See `docs/HARD_RULES.md` §6.
...
- [`docs/CI.md`](./docs/CI.md) — GitHub Actions checks, blocking vs notice-only jobs.
- [`docs/REVENUECAT_SETUP.md`](./docs/REVENUECAT_SETUP.md) — mobile paywall env vars and key prefixes.
