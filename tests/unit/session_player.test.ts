...
import {
  ...
  seededRoutines,
  startSession,
} from "../../apps/mobile/lib/session-player";
...
describe("session player leftover wiring", () => {
  test("keeps breathwork on routines and mounts a sibling player", () => {
    const routines = readFileSync(
      join(import.meta.dir, "../../apps/mobile/app/(tempo)/routines.tsx"),
      "utf8",
    );
    const layout = readFileSync(
      join(import.meta.dir, "../../apps/mobile/app/(tempo)/_layout.tsx"),
      "utf8",
    );
    expect(routines).toContain("BreathworkTimer");
    expect(routines).toContain("/session-player");
    expect(layout).toContain('name="session-player"');
  });
});
