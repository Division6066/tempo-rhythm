...
- PRs run: `bun run lint`, `bun run typecheck`, `bun run test`, `bun run scan:forbidden-tech`, `bun run convex:schema-guard`, `bun run scan:ram-only-audit`, `bun run scan:design-tokens`. See [`docs/CI.md`](./CI.md) for which jobs are blocking today.
...
- **Secret scanning** runs in CI (Gitleaks + TruffleHog via `.github/workflows/security.yml`). Never merge a PR where the scan fires.
