...
- Document the scanner entry point with a `// RAM-ONLY` comment and include a linter check (`bun run scan:ram-only-audit` when implemented) that fails if a scanner function persists raw content.
...
- PRs run: `bun run lint`, `bun run typecheck`, `bun run test`, plus scan scripts when implemented (`scan:forbidden-tech`, `convex:schema-guard`, `scan:ram-only-audit`, `scan:design-tokens`). See `docs/CI.md` for which jobs block merge today.
...
- **Secret scanning** runs in CI via the Security workflow (Gitleaks + TruffleHog). Never merge a PR where the scan fires.
