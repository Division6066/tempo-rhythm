...
- Every AI routing function (`convex/lib/ai_router.ts`, confidence router) is unit-tested with a golden fixture set.
- PRs run: `bun run lint`, `bun run typecheck`, `bun test`. Additional scan scripts (`scan:forbidden-tech`, `convex:schema-guard`, `scan:ram-only-audit`, `scan:design-tokens`) are CI targets as they land in `package.json`.
