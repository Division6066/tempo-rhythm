...
- PRs run: `bun run lint`, `bun run typecheck`, `bun run test`, plus policy scans when implemented (`scan:forbidden-tech`, `scan:ram-only-audit`, `scan:design-tokens`). See `docs/CI.md` for which jobs block merge today.
