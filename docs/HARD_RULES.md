...
- Document the scanner entry point with a `// RAM-ONLY` comment and include a linter check (`bun run scan:ram-only-audit`, planned) that fails if a scanner function persists raw content.
...
- PRs run CI via Bun. **Blocking today:** `bun run typecheck`, `bun run lint`, Gitleaks + TruffleHog (see `docs/CI.md`). **Planned / notice-only until scripts land:** `bun run test`, `bun run scan:forbidden-tech`, `bun run convex:schema-guard`, `bun run scan:ram-only-audit`, `bun run scan:design-tokens`.
...
- **Secret scanning** runs in CI via Gitleaks and TruffleHog (`.github/workflows/security.yml`). Never merge a PR where either scan fires.
