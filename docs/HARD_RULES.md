...
- PRs run: `bun run lint`, `bun run typecheck`, `bun test`, and (when implemented) `bun run scan:forbidden-tech`, `bun run convex:schema-guard`, `bun run scan:ram-only-audit`, `bun run scan:design-tokens`. See [`docs/CI.md`](./CI.md) for which jobs block merge today.
- Typecheck and lint must pass before merge. Scan scripts are notice-only in CI until they land in `package.json`.
