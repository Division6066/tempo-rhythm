...
- PRs run: `bun run lint`, `bun run typecheck`, `bun test`, `bun run scan:forbidden-tech`, `bun run convex:schema-guard`, `bun run scan:ram-only-audit`, `bun run scan:design-tokens`.
- **Current CI** (see `docs/CI.md`): typecheck and lint block; tests run with `continue-on-error`; scan scripts log notices until implemented. Security workflow (Gitleaks + TruffleHog) blocks on verified secrets.
- Target: all of the above must pass before merge once Phase 3 scan scripts land.
...
- **Secret scanning** runs in CI via `.github/workflows/security.yml` (Gitleaks + TruffleHog). Never merge a PR where either scan fires.
