...
- PRs run: `bun run lint`, `bun run typecheck`, `bun run test`, `bun run scan:forbidden-tech`, `bun run convex:schema-guard`, `bun run scan:ram-only-audit`, `bun run scan:design-tokens`. See `docs/CI.md` for which jobs are blocking today.
