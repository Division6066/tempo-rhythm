...
- PRs run: `bun run lint`, `bun run typecheck`, `bun test`, and policy scans when implemented (`scan:forbidden-tech`, `scan:ram-only-audit`, `scan:design-tokens`). See [`docs/CI.md`](./CI.md) for which jobs block merge today.
