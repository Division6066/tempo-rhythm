...
- **iOS Simulator:** macOS only. Windows/Linux developers use Expo Go on a physical iPhone or EAS cloud builds.
- **Android:** Android Studio emulator or Expo Go on device.
- **RTL:** forced RTL is enabled in `app.json` for Hebrew UI experiments.

## Troubleshooting

| Issue | Fix |
|---|---|
| `eas build` cannot find project | Run from `apps/mobile/`; confirm `app.json` → `extra.eas.projectId` |
| Metro "cannot find module" after clone | `bun install` at repo root; `bun run dev` with `--clear` if needed |
| Wrong Convex deployment on device | Update `EXPO_PUBLIC_CONVEX_URL` in `.env.local` or EAS secret |
| Bundle ID mismatch in App Store Connect | Must match `com.temporhythm.app` exactly |

## Related docs

- `docs/REVENUECAT_SETUP.md` — subscription keys and webhook
- `docs/LOCAL_DEV_WINDOWS.md` — Windows + Git Bash setup for mobile
- `docs/ENVIRONMENTS.md` — dev vs deployment env contract
