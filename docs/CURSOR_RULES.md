...
- **AI SDKs:** All LLM calls go through `convex/lib/ai_router.ts` using native `fetch` against `https://api.mistral.ai/v1/chat/completions`. Direct provider SDKs add bundle weight and bypass the tier router. OpenRouter is no longer used.
...
**Rationale.** Arbitrary values are how design systems erode. Once one file uses `#ff7a00` instead of `text-accent`, the next agent copies it, and within two weeks you have 17 orange-ish oranges and no way to theme. The design-token enforcer (`scan:design-tokens`) will fail the build on arbitrary values once the script lands (see `docs/CI.md`).
