...
| AI SDKs | `openai`, `@anthropic-ai/sdk`, `@google/generative-ai`, `@mistralai/mistralai` | Mistral API via native `fetch` (`convex/lib/ai_router.ts`) |
...
- **AI SDKs:** All LLM calls go through `convex/lib/ai_router.ts` using native `fetch` against the Mistral chat-completions API. Direct provider SDKs are forbidden.
