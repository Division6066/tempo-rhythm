...
| AI SDKs | `openai`, `@anthropic-ai/sdk`, `@google/generative-ai`, `@mistralai/mistralai`, OpenRouter | Mistral API via native `fetch` in `convex/lib/ai_router.ts` |
...
- **AI SDKs:** Direct provider SDKs and OpenRouter are forbidden. LLM traffic flows through the Mistral API via native `fetch` in Convex actions (`convex/lib/ai_router.ts`), keeping one integration surface and HARD_RULES-aligned cost control.
