...
- **AI SDKs:** All LLM traffic goes through `convex/lib/ai_router.ts` against `https://api.mistral.ai/v1/chat/completions`. Direct vendor SDKs add bundle weight and bypass the tiered routing + confidence policies in HARD_RULES §6.
...
**Rationale.** If the Mistral API has an outage, we need to disable AI features gracefully rather than stacking failed requests. If RevenueCat has a billing incident, we need to pause paywall enforcement rather than lock users out. Startup-only flags do not help when a dev server has been running for a week.
