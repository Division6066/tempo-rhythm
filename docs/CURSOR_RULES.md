...
- **AI SDKs:** All LLM calls go through `convex/lib/ai_router.ts` using native `fetch` against `https://api.mistral.ai/v1/chat/completions`. Direct provider SDKs are forbidden; OpenRouter is not used.
