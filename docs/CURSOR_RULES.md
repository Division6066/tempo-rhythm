...
- **AI SDKs:** All LLM calls go through `convex/lib/ai_router.ts` using native `fetch` against the Mistral chat-completions API. Direct vendor SDKs add bundle weight and bypass tier/fallback policy in HARD_RULES §6.
