...
| AI SDKs | `openai`, `@anthropic-ai/sdk`, `@google/generative-ai` | Mistral API via native `fetch` in `convex/lib/ai_router.ts` |
...
- **AI SDKs:** All LLM calls go through `convex/lib/ai_router.ts` against `https://api.mistral.ai/v1/chat/completions`. Direct provider SDKs are forbidden. OpenRouter is no longer used.
