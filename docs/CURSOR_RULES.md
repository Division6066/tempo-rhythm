...
- **AI SDKs:** All LLM calls go through `convex/lib/ai_router.ts`, which POSTs to `https://api.mistral.ai/v1/chat/completions`. No direct provider SDKs. Tier escalation (`fast` → `balanced` → `deep`) is built into the router.
...
**Rationale.** Arbitrary values are how design systems erode. Once one file uses `#ff7a00` instead of `text-accent`, the next agent copies it, and within two weeks you have 17 orange-ish oranges and no way to theme. The design-token enforcer (`bun run scan:design-tokens`) fails the build on arbitrary values when implemented.
