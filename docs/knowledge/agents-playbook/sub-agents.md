...
| `tempo-test-runner` | **no** | after build completes | Runs `bun run typecheck`, `bun run test`, `bun run scan:forbidden-tech`, `bun run scan:ram-only-audit`, `bun run scan:design-tokens`. Reports pass/fail per suite. See `docs/CI.md` for which scans are blocking in CI today. |
