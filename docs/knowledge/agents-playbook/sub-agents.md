...
**Sub-agents** are scoped, single-purpose agents invoked by the main Cursor agent or the orchestrator during a feature build. They do verification, not implementation. Each lives at `.cursor/agents/<name>.md` (see also `docs/AGENT_AUTOMATION_RUNBOOK.md` for background automation agents).
...
| `tempo-test-runner` | **no** | after build completes | Runs `bun run typecheck`, `bun run lint`, `bun run test`, and scan scripts when present (`scan:forbidden-tech`, `scan:ram-only-audit`, `scan:design-tokens`). See `docs/CI.md` for which gates are blocking vs notice-only. Reports pass/fail per suite. |
