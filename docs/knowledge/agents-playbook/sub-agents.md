...
| `tempo-test-runner` | **no** | after build completes | Runs `bun run typecheck`, `bun test`, and scan scripts when present (`scan:forbidden-tech`, `scan:ram-only-audit`, `scan:design-tokens`). Reports pass/fail per suite. |
