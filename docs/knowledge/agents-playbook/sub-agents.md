...
| `tempo-test-runner` | **no** | after build completes | Runs `bun run typecheck`, `bun run test`, and policy scans when scripts exist (`scan:forbidden-tech`, `scan:ram-only-audit`, `scan:design-tokens`). Reports pass/fail per suite. |
